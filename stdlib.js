...
var lib = {
	...
	msdispatch: {
		oid: 0,
		emitter: _.extend({}, Events),

		__stringForEvent: function(obj, prop) {
			if (!obj || !obj.__msdoid)
				return null;

			var result = 'change:' + obj.__msdoid;
			if (prop != null)
				result += ':' + prop;
		...
		on: function(obj, prop, cb, ctx) {
			if (!obj.__msdoid)
				obj.__msdoid = (this.oid += 1);
			this.emitter.on(this.__stringForEvent(obj, prop), cb, ctx);
		},

		off: function(obj, prop, cb, ctx) {
		...
		eventStream: function(obj, prop, transformer) {
			if (!obj.__msdoid)
				obj.__msdoid = (this.oid += 1);
			return Bacon.fromEventTarget(this.emitter,
					this.__stringForEvent(obj, prop),
					transformer);
		},
